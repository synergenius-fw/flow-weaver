...
import { serveInfo, loadServe, toast, agents, loadAgents, flatSteps, openAgents, openDoc, openEndpoints, setHttpRoutes, type ParsedWorkflow, type HttpRoute, type HttpMethod } from '../state';
  ...
  return (
    <>
      <div class="card serve">
        ...
        {!s?.running && (
          <div class="in">
            <div class="hint">Start the server from a terminal in this project. It listens on loopback; add <code>--host 0.0.0.0 --token …</code> to reach it from elsewhere.</div>
            <div class="composed"><code>{s?.command ?? 'fw serve --trace'}</code><span class="sp" /><button class="linkish" onClick={() => copy(s?.command ?? 'fw serve --trace')}>copy</button></div>
          </div>
        )}
        ...
        <div class="in">
          ...
          {!editing && routes.map((r) => (
            <div class="declared" key={`${r.method} ${r.path}`}>
              ...
              <div class="formfoot"><button class="btn sm" onClick={() => copy(curlFor(url, w.name, r, w.params, params))}>copy request</button><span class="hint">parameters come from the run form</span></div>
            </div>
          ))}
        </div>
        ...
        <div class="in">
          ...
          {showResource && (
            <>
              ...
              <div class="formfoot"><button class="btn sm" onClick={() => copy(legacy)}>copy request</button></div>
            </>
          )}
        </div>
