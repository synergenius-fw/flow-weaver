...
The durable classification is optional for an ordinary node type and required for every node reachable from a workflow that contains a `@durableGate`. See [Durable Gates](durable-gates.md).
