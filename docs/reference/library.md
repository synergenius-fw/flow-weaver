...
- `await runs.checkResume({ runId, input })` throws what `resume` would refuse before running anything (`RunNotFoundError`, `RunNotWaitingError`, `BundleChangedError`, `MissingOutputsError`, `InvalidAnswerError`) and changes nothing. A service that resumes in the background calls it first, to answer its caller with the reason.
