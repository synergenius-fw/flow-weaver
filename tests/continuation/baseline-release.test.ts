...
describe("frozen Stitch continuation baseline dependency", () => {
  ...
  it("pins the exact candidate engine source tree", () => {
    const sourceTree = execFileSync("git", ["rev-parse", "HEAD:src"], {
      cwd: repositoryRoot,
      encoding: "utf8",
    }).trim();

    expect(sourceTree).toBe("0a9f42a683b0edf21a23e7d7fdae234de0dae4b9");
  });
});
