...
describe('Workflow Execution', () => {
  ...
  describe('State Isolation', () => {
    it('should handle multiple sequential executions', async () => {
      ...
      const result1 = await calculate(true, { a: 1, b: 2, factor: 3 }, testHelpers.createRuntime('calculate'));
      const result2 = await calculate(true, { a: 10, b: 20, factor: 2 }, testHelpers.createRuntime('calculate'));
      const result3 = await calculate(true, { a: 100, b: 200, factor: 1 }, testHelpers.createRuntime('calculate'));
  ...
  describe('Idempotency', () => {
    it('should produce consistent results for same inputs', async () => {
      ...
      const result1 = await calculate(true, params, testHelpers.createRuntime('calculate'));
      const result2 = await calculate(true, params, testHelpers.createRuntime('calculate'));
      const result3 = await calculate(true, params, testHelpers.createRuntime('calculate'));
