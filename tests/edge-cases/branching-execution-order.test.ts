...
describe('Branching Execution Order', () => {
  const branchingFile = path.join(__dirname, '../../fixtures/basic/example-branching.ts');
  const outputFile = path.join(global.testHelpers.outputDir, 'branching-execution-order.generated.ts');
  ...
  describe('Success Branch', () => {
    it('should execute processValid when validation succeeds', async () => {
      ...
      const result = await validateAndProcess(
        true,
        { input: { name: 'John Doe' } },
        testHelpers.createRuntime('validateAndProcess'),
      );
  ...
  describe('Failure Branch', () => {
    ...
    it('should handle missing name field', async () => {
      ...
      const result = await validateAndProcess(true, { input: {} }, testHelpers.createRuntime('validateAndProcess'));
