...
describe('Scoped Ports Workflow E2E', () => {
  ...
  it('should handle single item', async () => {
    const result = await module.scopedPortsWorkflow(
      true,
      { items: [5] },
      testHelpers.createRuntime('scopedPortsWorkflow'),
    );
  ...
  it('should have correct port ordering (mandatory ports first)', () => {
    ...
    const forEach = parseResult.nodeTypes.find((nt: TNodeTypeAST) => nt.functionName === 'forEach');
