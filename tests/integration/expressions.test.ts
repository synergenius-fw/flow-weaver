...
describe('Expression-Based Values', () => {
  const exampleFile = path.join(__dirname, '../../fixtures/basic/example-expressions.ts');
  const outputFile = path.join(global.testHelpers.outputDir, 'expressions-test.generated.ts');
  ...
  it('should execute with default values (input=10, multiplier=2, offset=0)', async () => {
    ...
    const result = await expressionsWorkflow(true, { input: 10 }, testHelpers.createRuntime('expressionsWorkflow'));
  ...
  it('should use default values when inputs not connected (input=5)', async () => {
    ...
    const result = await expressionsWorkflow(true, { input: 5 }, testHelpers.createRuntime('expressionsWorkflow'));
  ...
  it('should handle optional inputs (input=7)', async () => {
    ...
    const result = await expressionsWorkflow(true, { input: 7 }, testHelpers.createRuntime('expressionsWorkflow'));
