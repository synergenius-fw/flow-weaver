...
describe('Scoped Workflows', () => {
  ...
  it('should execute scoped workflow correctly', async () => {
    ...
    // Test execution
    const result = await module.scopedWorkflow(true, { value: 5 }, testHelpers.createRuntime('scopedWorkflow'));
  ...
  it('should handle multiple scoped children', async () => {
    ...
    lines.push(` */`);
    lines.push(
      `function scopeNode(execute: boolean, x: number) { if (!execute) return { onSuccess: false, onFailure: false, x: 0 }; return { onSuccess: true, onFailure: false, x }; }`,
    );
    lines.push(``);
    ...
    lines.push(` */`);
    lines.push(
      `function child1(execute: boolean, val: number) { if (!execute) return { onSuccess: false, onFailure: false, val: 0 }; return { onSuccess: true, onFailure: false, val: val + 1 }; }`,
    );
    lines.push(``);
    ...
    lines.push(` */`);
    lines.push(
      `function child2(execute: boolean, val: number) { if (!execute) return { onSuccess: false, onFailure: false, val: 0 }; return { onSuccess: true, onFailure: false, val: val * 2 }; }`,
    );
    lines.push(``);
    ...
    lines.push(` */`);
    lines.push(
      `function child3(execute: boolean, val: number) { if (!execute) return { onSuccess: false, onFailure: false, val: 0 }; return { onSuccess: true, onFailure: false, val: val + 10 }; }`,
    );
    lines.push(``);
    ...
    lines.push(` */`);
    lines.push(
      `export async function multiChildScope(execute: boolean, params: { x: number }): Promise<{ onSuccess: boolean; onFailure: boolean; result: number }> {`,
    );
    ...
    try {
      ...
      const result = await module.multiChildScope(true, { x: 10 }, testHelpers.createRuntime('multiChildScope'));
