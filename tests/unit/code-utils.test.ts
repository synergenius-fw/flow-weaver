...
describe('Code Generation Utilities', () => {
    ...
    it('should not rethrow when onFailure connection exists', () => {
      ...
      const workflow = createWorkflow({
        connections: [
          ...
          {
            type: 'Connection',
            from: { node: 'testNode', port: 'onFailure' },
            to: { node: 'node2', port: 'execute' },
          },
        ],
      });
      const lines: string[] = [];
  ...
  describe('buildExecutionContextReturnForBranch', () => {
    it('should build return object from exit connections', () => {
      ...
      const returnStr = buildExecutionContextReturnForBranch(workflow, lines, true, 'main', '  ', ['node1']);
  ...
  describe('removed global node identity', () => {
    it('should not emit process-global node identity state', () => {
      const node = createNodeType();
      const workflow = createWorkflow();
      const lines: string[] = [];
      ...
      const code = lines.join('\n');
      expect(code).not.toContain('__fw_current_node_id__');
    });
  });
});
