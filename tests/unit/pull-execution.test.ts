...
describe('Pull Execution', () => {
  const inputFile = path.join(__dirname, '../../fixtures/advanced/example-pull.ts');
  const outputFile = path.join(global.testHelpers.outputDir, 'example-pull.generated.ts');

  beforeAll(async () => {
    const generatedCode = await generator.generate(inputFile, 'pullExecutionWorkflow');
  ...
  it('should execute with pull semantics (input = 5 → result = 25)', async () => {
    ...
    //           result = 10 + 15 = 25
    const result = await generatedModule.pullExecutionWorkflow(
      true,
      { input: 5 },
      testHelpers.createRuntime('pullExecutionWorkflow'),
    );
  ...
  it('should produce same result on second run with same input', async () => {
    ...
    const result1 = await generatedModule.pullExecutionWorkflow(
      true,
      { input: 5 },
      testHelpers.createRuntime('pullExecutionWorkflow'),
    );
    const result2 = await generatedModule.pullExecutionWorkflow(
      true,
      { input: 5 },
      testHelpers.createRuntime('pullExecutionWorkflow'),
    );
