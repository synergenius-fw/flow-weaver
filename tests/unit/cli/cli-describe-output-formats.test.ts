...
describe('describeCommand coverage', () => {
  ...
  it('should handle --compile flag', async () => {
    ...
    // --compile writes the generated body into the file, then describes it.
    expect(fs.readFileSync(filePath, 'utf8')).toContain('DO NOT EDIT - This section is auto-generated by Flow Weaver');
    expect(out.text()).toContain('Updated runtime markers in describe-compile.ts');
    const json = JSON.parse(out.of('log').slice(out.of('log').indexOf('{')));
    expect(json.name).toBe('simpleWf');
  });
});
