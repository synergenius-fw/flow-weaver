...
`;
  ...
  it("should include debug hooks in dev mode (sync function)", () => {
    // Dev mode (production: false, the default) emits debug controller hooks.
    // For sync functions, hooks are called without await.
    const content = `
/**
 * @flowWeaver nodeType
 * @input x
