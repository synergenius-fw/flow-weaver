...
describe('Sync Pull Execution', () => {
  ...
  const outputFile = path.join(global.testHelpers.outputDir, 'example-sync-pull.generated.ts');

  let generatedCode: string;

  beforeAll(async () => {
    generatedCode = await generator.generate(inputFile, 'syncPullWorkflow', {
      production: true,
    });
  ...
  describe('runtime execution', () => {
    it('should execute sync pull workflow without throwing async error', async () => {
      ...
      try {
        const result = generatedModule.syncPullWorkflow(
          true,
          { input: 5 },
          testHelpers.createRuntime('syncPullWorkflow'),
        );
    ...
    it('should produce correct result with different inputs', async () => {
      ...
      const result1 = generatedModule.syncPullWorkflow(
        true,
        { input: 3 },
        testHelpers.createRuntime('syncPullWorkflow'),
      );
      const result2 = generatedModule.syncPullWorkflow(
        true,
        { input: 10 },
        testHelpers.createRuntime('syncPullWorkflow'),
      );
    ...
    it('should handle multiple executions correctly', async () => {
      ...
      const results = [1, 2, 3, 4, 5].map(
        (n) =>
          generatedModule.syncPullWorkflow(true, { input: n }, testHelpers.createRuntime('syncPullWorkflow')).result,
      );
