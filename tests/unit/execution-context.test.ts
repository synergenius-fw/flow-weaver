...
function createContext(isAsync = true, abortSignal?: AbortSignal) {
  return new GeneratedExecutionContext(isAsync, testHelpers.createRuntime('executionContext', {}, abortSignal));
}
...
describe('GeneratedExecutionContext', () => {
  describe('Variable Storage', () => {
    ...
    it('should reset local bookkeeping without erasing durable state', async () => {
      const ctx = createContext(true);
      const idx = ctx.addExecution('node1');
      await ctx.setVariable({ id: 'node1', portName: 'result', executionIndex: idx }, 42);
  ...
  describe('Scope Management', () => {
    describe('createScope', () => {
      it('should create isolated scope with cleanScope=true', async () => {
        ...
        const nestedInheritingScope = scopedCtx.createScope('nested', 0, 'block', false);
        expect(
          nestedInheritingScope.hasVariable({
            id: 'parent',
            portName: 'data',
            executionIndex: idx,
          }),
      ...
      it('should inherit variables with cleanScope=false', async () => {
        ...
        const value = await scopedCtx.getVariable({
          id: 'parent',
          portName: 'data',
          executionIndex: idx,
        });
      ...
      it('should inherit execution counter', () => {
        ...
        // Exact addresses count occurrences per node, not globally.
    ...
    describe('mergeScope', () => {
      ...
      it('should update execution counter after merge', () => {
        ...
        // A distinct node starts at occurrence zero.
        const nextIdx = ctx.addExecution('afterMerge');
        expect(nextIdx).toBe(0);
        expect(ctx.getExecutionCount()).toBe(5);
      });
    });
  });
  ...
  describe('Sync vs Async Mode', () => {
    it('should return Promise in async mode', async () => {
      ...
      await setResult;
      const getResult = ctx.getVariable({
        id: 'node1',
        portName: 'result',
        executionIndex: idx,
      });
  ...
  describe('Function Values', () => {
    ...
    it('should handle async function values', async () => {
      ...
      await ctx.setVariable({ id: 'node1', portName: 'result', executionIndex: idx }, async () => 'async computed');
  ...
  describe('Cancellation', () => {
    ...
    it('should return false for isAborted() when signal not aborted', () => {
      ...
      const ctx = createContext(true, controller.signal);
      expect(ctx.isAborted()).toBe(false);
    });
