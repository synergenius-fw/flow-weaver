...
describe('Pull Execution Node ID Bug', () => {
  ...
  const outputFile = path.join(global.testHelpers.outputDir, 'pull-node-id-bug.generated.ts');
  ...
  it('should execute correctly when instance ID differs from node type name', async () => {
    ...
    const result = await generatedModule.testPullNodeId(
      true,
      { a: 5, b: 10 },
      testHelpers.createRuntime('testPullNodeId'),
    );
