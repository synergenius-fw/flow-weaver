...
export const CLI_COMMANDS: TCliCommandDoc[] = [
  // ── Core commands ──────────────────────────────────────────────
  {
    ...
    options: [
      ...
      { flags: '-w, --workflow', arg: '<name>', description: 'Compile only this workflow (default: every workflow in the file)' },
      ...
      { flags: '--clean', description: 'Omit redundant @param/@returns annotations from compiled output' },
  ...
  },
  {
    ...
    botCompatible: true,
    options: [
      { flags: '-w, --workflow', arg: '<name>', description: 'Validate only this workflow (default: every workflow in the file)' },
  ...
  },
  {
    ...
    botCompatible: true,
    options: [
      { flags: '-f, --format', arg: 'json|text|mermaid|paths|ascii|ascii-compact', description: 'Output format (paths: enumerate all Start-to-Exit paths via DFS; ascii: a terminal diagram)' },
      { flags: '-w, --workflow', arg: '<name>', description: 'Target workflow' },
  ...
  },
  {
    ...
    options: [
      ...
      { flags: '-s, --stream', description: 'Stream trace events in real time' },
      { flags: '--json', description: 'Output result as JSON' },
      { flags: '--timeout', arg: '<ms>', description: 'Execution timeout in milliseconds' },
      { flags: '--mocks', arg: '<json>', description: 'Mock config as JSON: gates (answers by node id), events, agents, invocations, fast', exclusive: 'mocks' },
      { flags: '--mocks-file', arg: '<path>', description: 'Path to JSON file with mock config for built-in nodes', exclusive: 'mocks' },
      { flags: '-d, --debug', description: 'Start in step-through debug mode' },
      { flags: '-b, --breakpoint', arg: '<nodeIds...>', description: 'Set initial breakpoints (repeatable)' },
    ],
  },
  {
    name: 'serve',
    ...
    options: [
      ...
      { flags: '--production', description: 'Deprecated: the same as leaving --trace off' },
    ],
  },
  ...
  },
  {
    ...
    group: 'create',
    options: [
      { flags: '-t, --template', arg: nodeTemplates.map(t => t.id).join('|'), description: 'Node template to use', defaultValue: 'transformer' },
  ...
  // ── Project & dev commands ─────────────────────────────────────
  {
    ...
    options: [
      ...
      { flags: '--preset', arg: 'nocode|vibecoder|lowcode|expert', description: 'User preset' },
      { flags: '--use-case', arg: 'data|ai|api|automation|minimal', description: 'Use case' },
      { flags: '--mcp / --no-mcp', description: 'Configure MCP for AI editors after scaffolding, or skip the prompt' },
      { flags: '--no-agent', description: 'Skip the post-init agent launch prompt' },
      ...
      { flags: '--json', description: 'Output a report as JSON' },
    ],
  },
  {
    name: 'watch',
    ...
    options: [
      ...
      { flags: '-p, --production', description: 'Production build (no debug events)' },
      { flags: '-s, --source-map', description: 'Generate source maps' },
      { flags: '-w, --workflow', arg: '<name>', description: 'Target specific workflow' },
      { flags: '-f, --format', arg: 'esm|cjs|auto', description: 'Module format', defaultValue: 'auto' },
      { flags: '--verbose', description: 'Detailed output' },
    ],
  },
  ...
  // ── Export & generation ────────────────────────────────────────
  {
    ...
    options: [
      ...
      { flags: '-p, --production', description: 'Production mode (no debug events)', defaultValue: 'false' },
      { flags: '--bundle', description: 'Bundle node types into the output' },
      ...
      { flags: '--durable-steps', description: 'Use deep generator with per-node durable steps' },
  ...
  },
  {
    ...
    options: [
      ...
      { flags: '-w, --workflow', arg: '<name>', description: 'Specific workflow to compare' },
