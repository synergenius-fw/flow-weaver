/**
 * The `fw` command-line program: every built-in command, its arguments and
 * options, and the error handling around its actions.
 *
 * Building the program has no side effects, so tests and scripts can load it
 * for its metadata or parse argument vectors against it. The entry point
 * (index.ts) owns what happens on a real invocation: the no-args welcome,
 * pack-contributed commands and parsing `process.argv`.
 */
...
// Version is injected at build time by esbuild (scripts/build-cli.ts)
declare const __CLI_VERSION__: string;
export const cliVersion = typeof __CLI_VERSION__ !== 'undefined' ? __CLI_VERSION__ : '0.0.0-dev';

/** Build the `fw` program with every built-in command registered. */
